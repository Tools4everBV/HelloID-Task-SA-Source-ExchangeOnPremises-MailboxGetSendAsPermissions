...
try {
    ...
    try {
        ...
        Write-Information "Querying SendAs permissions to mailbox [$($exchangeQuerySplatParams.Identity)]"
        $mailboxPermissions = Get-RecipientPermission @exchangeQuerySplatParams -ErrorAction Stop

        $mailboxPermissions = $mailboxPermissions | Sort-Object -Property User
        ...
        throw "Error querying SendAs permissions to mailbox [$($exchangeQuerySplatParams.Identity)]. Error Message: $($errorMessage.AuditErrorMessage)"
    }
}
catch {
    $ex = $PSItem
    ...
    Write-Error "Error querying SendAs permissions to mailbox [$($mailboxGuid)]. Error Message: $($errorMessage.AuditErrorMessage)"
}